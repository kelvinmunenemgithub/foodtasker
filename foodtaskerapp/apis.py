...
def driver_pick_order(request):
    ...
    if request.method == "POST":
        ...
        # Check if driver can only pick up one order at same time
        if Order.objects.filter(driver=driver).exclude(status=Order.ONTHEWAY):
            return JsonResponse({"status": "fail", "error": "You can only pick one order"})
        ...
        try:
            ...
            return JsonResponse({"status": "failed", "error": "This order has been picked by another driver"})
